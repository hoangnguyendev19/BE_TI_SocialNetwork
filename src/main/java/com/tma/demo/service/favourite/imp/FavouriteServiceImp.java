...
public class FavouriteServiceImp implements FavouriteService {
    ...
    public Page<UserResponse> getFavouritePosts(PagingRequest<PostFilter> pagingRequest) {
        ...
        List<UserResponse> userRespons = pageUser.stream().map(user -> new UserResponse(user.getId().toString(), user.getFirstName(), user.getLastName()))
        .toList();
        return new PageImpl<>(userRespons, pageable, pageUser.getTotalElements());
    }
